package com.myteammanager.util;

public class KeyConstants {

	public static final String INTENT_BEAN_ADDED = "com.myteammanager.BEAN_ADDED";
	public static final String INTENT_BEAN_CHANGED = "com.myteammanager.BEAN_CHANGED";
	public static final String INTENT_BEAN_DELETED = "com.myteammanager.BEAN_DELETED";
	public static final String INTENT_RELOAD_LIST = "com.myteammanager.RELOAD8LIST";

	public static final String KEY_BEANDATA_LIST = "beandata_list";
	public static final String KEY_BEANDATA = "beandata";
	public static final String KEY_EVENT_OR_MATCH = "event_or_match";
	public static final String KEY_MATCH = "match";
	public static final String KEY_PLAYER = "player";
	public static final String KEY_PLAYERS_LIST = "players";
	public static final String KEY_NOT_CHOSEN_PLAYERS = "notChosenPlayer";
	public static final String KEY_PLAYERS_ALREADY_IN_THE_LINEUP = "playersLineUp";
	public static final String KEY_CHOSEN_CONTACTS = "chosenContacts";
	public static final String KEY_CHOSEN_CONTACT = "chosenContact";
	public static final String KEY_SELECTED_RECIPIENT = "selectedRecipients";
	public static final String KEY_MSG_TEXT = "msgText";
	

	public static final int CODE_BEAN_CHANGE = 1;
	public static final int CODE_PLAYER_DETAILS_EDITED = 2;
	public static final int CODE_PLAYER_FOR_LINEUP_CHOSEN = 3;
	public static final int CODE_PLAYERS_FOR_BENCH_CHOSEN = 4;
	public static final int CODE_SETTINGS_ACTIVIY = 5;
	public static final int CODE_CONTACTS_CHOSEN = 6;
	public static final int WIZARD_TEAM_NAME_CODE = 7;
	public static final int WIZARD_ENTER_PLAYERS_INFO_CODE = 8;
	public static final int WIZARD_ENTER_PLAYERS_PROCESS_COMPLETED_CODE = 9;
	public static final int WIZARD_START_CHOOSE_CONTACTS = 10;
	public static final int CODE_CONTACT_CHOSEN = 11;
	public static final int CODE_SIGNUP_ACTIVITY = 12;
	public static final int CODE_LOGIN_ACTIVITY = 13;
	public static final int CODE_FACEBOOK_ACTIVITY = 14;

	public static final int RESULT_BEAN_ADDED = 1;
	public static final int RESULT_BEAN_EDITED = 2;
	public static final int RESULT_BEAN_DELETED = 3;
	
	public static final int RESULT_PLAYER_FOR_LINEUP_CHOSEN = 400;
	public static final int RESULT_PLAYERS_ONTHEBENCH_CHOSEN = 401;
	public static final int RESULT_SIGNUP_DONE = 3200;
	public static final int RESULT_PLAYER_DETAILS_EDITED = 8658;
	public static final int RESULT_CONTACTS_CHOSEN = 8659;
	public static final int RESULT_CONTACT_CHOSEN = 8660;
	public static final int RESULT_START_CONTACTS_SELECTION = 8661;
	
	public static final String FACEBOOK_APP_ID = "365945950147624";

}
